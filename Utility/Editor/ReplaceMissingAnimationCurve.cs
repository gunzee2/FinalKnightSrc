	...
	void FindPath ()
	{
		...
		foreach (var clip in clips) {
			...
			foreach (var binding in bindings) {
				// 追加：コンポーネントがあるかを確認
				var foundTransform = animator.transform.Find (binding.path);
				if (foundTransform != null && (foundTransform == null || foundTransform.GetComponent(binding.type) != null)) continue;
